...
A structured system for aligning daily work with long-term goals. The AI's job is to help the user run workflows — reflecting, processing meeting notes, synthesizing patterns, drafting communications — all grounded in the user's personal context files.
...
prompts/          Copy-paste prompts (reference only — commands are in .claude/commands/)
...
- Personal content (Meeting Notes, Reflections, My Team, etc.) is gitignored — don't worry about committing it
